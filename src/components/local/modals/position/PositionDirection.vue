...
<template>
  <div :class="$style.wrapper">
    <div :class="$style.from">
      <div :class="$style.crc">
        <img :src="`https://services.tzkt.io/v1/avatars/${accountStore.pkh}`" :class="$style.image" alt="avatar" />
      </div>
      ...
      <div :class="$style.meta">
        ...
        <div :class="$style.subname">
          <span @click="amount.value = Math.floor(accountStore.balance)"
            @dblclick="amount.value = accountStore.balance / 2">{{ accountStore.balance }}</span>
          ꜩ
        </div>
      </div>
    </div>
